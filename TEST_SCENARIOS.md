...
### 3.4 プレミアムコンテンツ表示（有料会員 / トライアル中）

1. `subscription_status` が `ACTIVE` または `TRIALING` のユーザーでログインする
2. 有料コンテンツを含む記事にアクセスする
3. 初期表示で無料部分とペイウォールが表示されることを確認
4. 認証状態確認後に有料コンテンツが表示されることを確認
5. ペイウォールが非表示になることを確認

**期待結果:**

- 認証確認後に有料コンテンツが表示される
- ペイウォールが消える
- `ACTIVE` と `TRIALING` の両方で同じ挙動になる
...
### 4.4 加入済みユーザーのプランページ表示

1. `subscription_status` が `ACTIVE` または `TRIALING` のユーザーでログインする
2. `/subscription` にアクセスする
3. 各プランの加入ボタンが無効化されることを確認
4. ボタン文言が「加入済み」になることを確認

**期待結果:**

- 加入済みユーザーは新規 Checkout に進めない
- `ACTIVE` と `TRIALING` の両方で「加入済み」扱いになる
